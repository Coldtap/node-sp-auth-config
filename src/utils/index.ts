...
export const convertAuthContextToSettings = (
  ...
  const plainContext: IAuthContextSettings = {
...
export const convertSettingsToAuthContext = (
  configObject: IAuthContextSettings,
  settings: IAuthConfigSettings = {}
): IAuthContext => {
...
export const isOnPrem = (siteUrl: string): boolean => {
  ...
  return (
    ['.sharepoint.com', '.sharepoint.cn', '.sharepoint.de', '.sharepoint-mil.us', '.sharepoint.us'].filter(
      (uri) => host.indexOf(uri) !== -1
    ).length === 0
  );
};
